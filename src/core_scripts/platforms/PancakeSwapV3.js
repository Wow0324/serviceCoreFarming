...
data = fs.readFileSync(__dirname + "/abi/PancakeswapRouterV3.json", 'utf-8');
const Routerabi = JSON.parse(data);
data = fs.readFileSync(__dirname + "/abi/PancakeswapFactoryV3.json", 'utf-8');
...
const fee_decimal = 4;
const {
    PancakeswapGetFeeFromPairName,
    readLPInformation,
    writeLPInformation,
    PancakeswapGetpIdFromPairName,
...
//=========================================================
const ExchangeFactoryAddr = Addresses.PancakeSwapExchangeFactoryV3;
const ExchangeRouterAddr = Addresses.PancakeSwapExchangeRouterV3;
const ExchangeRouterAddrV2 = Addresses.PancakeSwapExchangeRouter;
const cakeVaultAddr = Addresses.PancakeSwapcakeVault;
const masterchefAddr = Addresses.PancakeSwapmasterchefV3;
const UsdtAddr = Addresses.PancakeSwapUsdt;
...
exports.statusGet = async (req) => {
    ...
    const { addr1, addr2 } = PancakeswapGetAddressFromPairName(pair);
    ...
    try {
        ...
        var fee = PancakeswapGetFeeFromPairName(pair);
        ...
        if (pool == "1") {
            ...
            try {
                ...
                try {
                    if (UsdtAddr != addre1) {
                        price1 = await exchangeRouterV2.getAmountsIn(ethers.utils.parseUnits(String("1"), decimals), [UsdtAddr, addre1]);
...
exports.liquidityAdd = async (req) => {
    ...
    try {
        ...
        try{
            ...
            res["responseData"]['result'] = { tokenId: args.tokenId.toNumber(), liquidity: ethers.utils.formatEther(String(args.liquidity)), amount0: ethers.utils.formatEther(String(args.amount0)), amount1: ethers.utils.formatEther(String(args.amount1)) };
