...
const {
    readLPInformation,
    writeLPInformation,
    SpookySwapGetpIdFromPairName,
    SpookySwapGetAddressFromPairName
} = require("./utils/getAddressFn");
const ExchangeFactoryAddr = Addresses.SpookySwapExchangeFactory;
const ExchangeRouterAddr = Addresses.SpookySwapExchangeRouter;
const cakeVaultAddr = Addresses.SpookySwapVault;
const masterchefAddr = Addresses.SpookySwapmasterchef;
const UsdtAddr = Addresses.SpookySwapUsdc;
const fUsdtAddr = Addresses.SpookySwapFUsdt;
...
exports.statusGet = async (req) => {
    ...
    const { addr1, addr2 } = SpookySwapGetAddressFromPairName(pair);
    ...
    try {
        ...
        if (pool == "1") {
            try {
                ...
                const decimals = 15;//await LPPair.decimals();
                ...
                var liquidity1_value = parseFloat(ethers.utils.formatEther(String(liquidity1Bg_value)));
                var liquidity2_value = parseFloat(ethers.utils.formatEther(String(liquidity2Bg_value)));
                ...
                var isprice1 = 0;
                var isprice2 = 0;
                var deciaml_add = 1000000000000000;
                var deciaml_add_LP = 1000000000000;
                try {
                    if (UsdtAddr != addre1) {
                        ...
                        liquidity1 = liquidity1 * deciaml_add_LP;
                    }
                }
                catch (err) {
                    isprice1 = 0;
                ...
                try {
                    if (UsdtAddr != addre2) {
                        ...
                        liquidity2 = liquidity2 * deciaml_add_LP;
                    }
                }
                catch (err) {
                    isprice2 = 0;
                ...
                if (addre1 == fUsdtAddr) { price1 = 0; price2 = price2 / 2; }
                if (addre2 == fUsdtAddr) { price2 = 0; price1 = price1 / 2; }
                //console.log("liquidity1_value=" + String(liquidity1_value) + ", liquidity2_value=" + String(liquidity2_value));
                console.log("liquidity1=" + String(liquidity1) + ", liquidity2=" + String(liquidity2));
                ...
                res["responseData"]["pool"]["LPTokens"] = ethers.utils.formatEther(String(balance));
        ...
        if (farm == "1") {
            try {
                ...
                var pid = SpookySwapGetpIdFromPairName(pair);
...
exports.liquidityAdd = async (req) => {
    ...
    try {
        const exchangeRouter = new ethers.Contract(ExchangeRouterAddr, Routerabi, owner);
        const token1 = new ethers.Contract(addre1, ["function approve(address spender, uint256 amount) public returns (bool)"], owner); //
        ...
        const token2 = new ethers.Contract(addre2, ["function approve(address spender, uint256 amount) public returns (bool)"], owner); //
